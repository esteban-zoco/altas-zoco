...
export const PaidOrganizerStepThree = () => {
  const {
    state,
  ...
  const handleFinalSubmit = async () => {
    ...
    if (!acceptedTerms) {
      setErrorMessage(
        "Tienes que aceptar la declaracion jurada y los terminos para enviar la solicitud.",
      );
      return;
    }
    ...
    if (totalBytes > MAX_EFFECTIVE_UPLOAD_BYTES) {
      ...
      if (largestKey) {
        setDocumentError(
          largestKey,
          "Este archivo es el mas pesado. Reducelo o reemplazalo por una version mas liviana.",
        );
      }
      setErrorMessage(
        `La documentacion supera el tamano maximo (${sizeLimitLabel}). Reduce los archivos e intenta nuevamente.`,
    ...
    try {
      ...
      const response = await fetch("/api/paid-organizer-submit", {
        method: "POST",
        body,
      });
      if (!response.ok) {
        if (response.status === 413) {
          ...
          if (largestKey) {
            setDocumentError(
              largestKey,
              "El servidor rechazo la carga por tamano. Prueba reduciendo este archivo (es el mas pesado).",
            );
          }
          throw new Error(
            `La documentacion supera el limite permitido por el servidor (${sizeLimitLabel}). Reduce el peso de los archivos e intenta nuevamente.`,
          );
        }
  ...
  return (
    <div className="space-y-6">
      <Alert
        title="Sube estos documentos para que podamos activar los cobros del organizador."
        description={`Puedes cargar archivos en cualquier formato. Limite total: ${sizeLimitLabel}. Si necesitas tiempo, guarda tu progreso y sigue despues.`}
      />
      <StepLayout
        step={3}
        title="Documentacion"
        description="Verificamos esta informacion para habilitar los cobros."
        onBack={previousStep}
        primaryButton={{
      ...
      >
        <div className="space-y-6">
          {isNatural ? (
            <>
              <FileUploadItem
                title="DNI frente"
                description="Sube la cara frontal del DNI."
              ...
              <FileUploadItem
                title="DNI dorso"
                description="Sube la cara trasera del DNI."
                accept={ACCEPT_ANY_FILE}
              ...
              <FileUploadItem
                ...
                description="Sube una constancia vigente emitida por AFIP o ARCA."
                accept={ACCEPT_ANY_FILE}
              ...
              <FileUploadItem
                ...
                description="Adjunta la constancia provincial vigente."
                accept={ACCEPT_ANY_FILE}
            ...
            <>
              ...
              <FileUploadItem
                ...
                description="Sube la constancia provincial obligatoria."
                accept={ACCEPT_ANY_FILE}
        ...
        <div className="space-y-3 rounded-3xl border border-slate-200 p-4">
          ...
          <p className="text-sm text-slate-600">
            ...
            calidad de declaracion jurada y asumiendo toda la responsabilidad
            civil, penal y administrativa por cualquier falsedad u omision, que
            la informacion ingresada es veraz y exacta.
          </p>
